...
const UpdateProductivityForm = ({
  ...
  const [selectedDate, setSelectedDate] = useState(new Date(initialData.dateOfEntry));
  const [loading, setLoading] = useState(false);

  useEffect(() => {
    if (initialData) {
      // Populate the subject and timing rows from initialData
      const initialRows = initialData.subjects.map((subject, index) => ({
        subject,
        timing: initialData.timing[index]
      }));
      setRows(initialRows);
    }
  }, [initialData]);
  ...
  const handleSubmit = async () => {
    ...
    const timingStrings = rows.map((row) => row.timing);
    ...
    // Make a network call to update productivity data (replace with your actual API URL)
    try {
      ...
      onUpdateProductivity(payload);
      setLoading(false);
      setRows([{ subject: '', timing: '' }]); // Reset the form

      // Display a beautiful alert upon successful addition
      alert('Productivity data updated successfully!'); // Customize the alert as needed
    } catch (error) {
  ...
  return (
    <Dialog open={open} onClose={onClose}>
      <DialogTitle>Update Productivity</DialogTitle>
      <DialogContent>
